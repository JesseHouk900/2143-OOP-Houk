...
class Player(object):
    ...
    def Roll(self):
        ...
        if self.TotalScore / self.TargetScore >= .8:
            self.SetStrategy('Sprint_To_Finish', self.Strategy)
    ...
    def CalcGoal(self,strategy):
        nums = []
        for x in range(self.Strategies[strategy]):
            nums.append(x)
        return self.NumDice * sum(nums)
        
    def Aggressive(self):
        Score, NumRolls = 0,0  
        strategy = 'Aggressive'
        Goal = self.CalcGoal(strategy)
        for x in range(self.Strategies[strategy]):
            if ((self.TotalScore + Score) < self.TargetScore and Score < Goal):
                NumRolls += 1
                roll = self.pig.Roll()
                if roll == 0:
                    break
    ...
    def SprintToFinish(self):
        Score = 0 
        NumRolls = 0 
        for i in range(self.Strategies['Sprint_To_Finish']):
            if ((self.TotalScore + Score) < self.TargetScore):
                NumRolls += 1
                roll = self.pig.Roll()
                if roll == 0:
